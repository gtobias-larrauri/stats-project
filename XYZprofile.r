XYZprofile<-function(ID){
  if(ID >= 202500000 || ID <=200000000) stop("Wrong LSE Student ID!")
  set.seed(ID)
  ...
  locations = c("Ashfield", "Ashford (London Middlesex)", "Atherton (Manchester)", "Banbury", "Barking (London)", "Barnet (London)", "Barnsley", 
   ...
   "Failsworth (Manchester)", "Farnborough", "Gateshead", "Gillingham", "Glasgow (Anniesland)", "Glasgow (Baillieston)", "Glasgow (Shieldhall)", "Gloucester", 
   "Goodmayes (London)", "Greenford (London)", "Guildford", "Halifax", "Hamilton", "Heckmondwike", "Hendon (London)", "Herne Bay", "High Wycombe", "Hither Green (London)",
   ...
   "Newport (Gwent)", "Norris Green (Liverpool)", "Northampton", "Norwich", "Nottingham (Colwick)", "Oxford (Cowley)", "Paisley", "Peterborough", "Pinner (London)", 
